...
const SideBar = () => {
  ...
  return (
    <aside className="bg-gray-900 text-white w-64 min-h-screen p-4">
      ...
      {["Users", "Gyms", "Reviews", "GYM Management", "A", "B", "C"].map(
        (item) => (
            ...
            {openSection === item && item === "GYM Management" && (
              <div className="ml-4 mt-2 space-y-2">
                <p className="text-sm cursor-pointer">
                  <Link href="/dashboard/gym-info">GYM Info</Link>
                </p>
                <p className="text-sm cursor-pointer">
                  <Link href="/dashboard/add-gym"> Add New Gym </Link>
                </p>
      ...
      <button className="mt-4 w-full bg-gray-700 py-2 rounded-md">
        Sign out
      </button>
    </aside>
  );
